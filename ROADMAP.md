...
Transform Memo Automator from a single-analyst productivity tool into a reliable, team-wide platform for updating Investment Committee (IC) PowerPoint memos from Excel proformas — with full auditability, consistent branding, and optional market data enrichment.
...
6. Review change log:
   ...
   - Check rejected items — fix proforma if needed
   - Review missed metrics — update manually if critical
...
```
memo_automator.py (2,532 lines — monolith)
├── extract_proforma()      — openpyxl data extraction
├── extract_schedule()      — mpxj/jpype schedule parsing
├── extract_memo()          — python-pptx content extraction
├── call_claude_mapping()   — Claude API: metric identification
├── call_claude_validation()— Claude API: QA cross-check
├── apply_updates()         — PPTX text/table modifications
├── apply_branding()        — theme + font + color reformat
├── normalize_layout()      — title/margin/TOC alignment
└── write_change_log()      — Markdown audit trail

app.py (560 lines)
└── Streamlit web dashboard ("Memo Chef")
```
...
|----|-----------|----------|--------|-------|
| TD-1 | **Monolithic module** — 2,500+ lines in one file | High | Medium | Split into packages: `extraction/`, `ai/`, `formatting/`, `output/` |
| TD-2 | **No test suite** — only 2 ad-hoc test scripts | High | Medium | Need pytest suite with fixtures, mocking, CI integration |
| TD-3 | **No CI/CD pipeline** | Medium | Low | GitHub Actions for lint + test on push |
| TD-4 | **Print-based logging** — no structured logging | Medium | Low | Replace with Python `logging` module, configurable levels |
| TD-5 | **Hardcoded prompt strings** — 600+ line prompts inline | Medium | Medium | Extract to `prompts/` directory as versioned templates |
| TD-6 | **No input validation** — trusts file formats | Medium | Low | Validate file extensions, sheet names, non-empty data |
| TD-7 | **No error recovery** — crash on API timeout/failure | Medium | Medium | Retry logic with exponential backoff, checkpoint/resume |
| TD-8 | **Magic numbers** — thresholds scattered in code | Low | Low | Move to config.yaml with documentation |
| TD-9 | **No type hints** — limited IDE support | Low | Low | Add type annotations incrementally |
| TD-10 | **Sandbox/test artifacts in repo** — `a. Sandbox/`, `x. Old/` | Low | Low | Clean up, .gitignore, or archive |
...
├── pipeline.py             # Orchestrator (step sequencing)
├── extraction/
│   ├── proforma.py         # Excel data extraction
│   ├── memo.py             # PPTX content extraction
│   └── schedule.py         # MPP schedule parsing
├── ai/
│   ├── client.py           # Claude API wrapper (retry, rate limit)
│   ├── mapping.py          # Metric mapping logic
│   └── validation.py       # QA validation logic
├── formatting/
│   ├── updates.py          # Apply text/table changes
│   ├── branding.py         # Theme + font + color
│   └── layout.py           # Layout normalization
├── output/
│   ├── changelog.py        # Change log generation
│   └── artifacts.py        # JSON/debug file output
├── prompts/
│   ├── mapping_v1.txt      # Mapping prompt template
│   └── validation_v1.txt   # Validation prompt template
└── market/                  # v1.5: Market data module
    ├── sources.py
    ├── schema.py
    └── ingestion.py
```
...
|------|-------|------|------------|
| Create ROADMAP.md | @brandon | 1d | — |
| Create CHECKLIST.md | @brandon | 0.5d | ROADMAP |
| Create CONTRIBUTING.md | @brandon | 0.5d | — |
| Create SECURITY.md | @brandon | 0.5d | — |
| Create RUNBOOK.md | @brandon | 0.5d | — |
| Create DATA_SOURCES.md | @brandon | 1d | — |
| Create initial ADRs | @brandon | 0.5d | — |
| Set up docs/ folder structure | @brandon | 0.5d | — |
| Add .gitignore improvements | @brandon | 0.25d | — |
...
| Task | Owner | Est. | Depends On |
|------|-------|------|------------|
| Add structured logging (replace print) | @developer | 2d | — |
| Add input file validation | @developer | 1d | — |
| Add proper error messages + graceful failures | @developer | 2d | Logging |
| Write pytest unit tests for extraction | @developer | 3d | — |
| Write pytest unit tests for update application | @developer | 2d | — |
| Create sample test fixtures (anonymized) | @developer | 1d | — |
| Add `--verbose` / `--quiet` CLI flags | @developer | 0.5d | Logging |
| Clean up sandbox/old artifacts | @developer | 0.5d | — |
| Update README with team onboarding steps | @brandon | 1d | — |
...
| Task | Owner | Est. | Depends On |
|------|-------|------|------------|
| Define market data schema (see DATA_SOURCES.md) | @brandon | 2d | — |
...
| `ROADMAP.md` | This document — project vision + phases | **New** |
...
```
┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│ Data Source   │────▶│ Ingestion    │────▶│ Validated    │
│ (API/CSV/DB) │     │ Pipeline     │     │ Data Store   │
└──────────────┘     │ - Fetch      │     │ (JSON/SQLite)│
                     │ - Normalize  │     └──────┬───────┘
                     │ - Validate   │            │
                     │ - Version    │            ▼
...
|---|------|-----------|--------|------------|
| R1 | **AI hallucination** — Claude maps wrong metric or invents values | Low | Critical | Two-stage validation; human review of change log; dry-run default |
| R2 | **API cost escalation** — large decks + batching = many API calls | Medium | Medium | Cost tracking, caching, prompt optimization, token budget alerts |
| R3 | **Proforma format drift** — teams use different tab/column layouts | High | Medium | Config profiles per project type; validation of expected tabs |
| R4 | **Breaking API changes** — Anthropic model updates change behavior | Low | High | Pin model versions in config; regression test suite |
| R5 | **Data sensitivity** — proforma data sent to external API | Medium | High | Document data handling in SECURITY.md; consider on-prem options |
| R6 | **Single point of failure** — one developer, no bus factor | High | High | Documentation, CONTRIBUTING.md, team training |
| R7 | **Font availability** — Pragmatica not installed on all machines | Medium | Low | Document requirement; PowerPoint auto-substitutes |
| R8 | **Market data licensing** — commercial data ToS violations | Medium | High | Legal review before integration; start with free sources |
| R9 | **Large file handling** — 100+ slide decks with embedded images | Low | Medium | Memory profiling; streaming extraction; page filtering |
| R10 | **Team adoption resistance** — analysts prefer manual workflow | Medium | Medium | Training docs, demo sessions, gradual rollout |
...
| **Dry run** | Preview mode — shows changes without modifying any files |
